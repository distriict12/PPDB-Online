...
const navItems: NavItem[] = [
  { icon: <GridIcon />, name: "Dashboard", path: "/siswa" },
  { icon: <ListIcon />, name: "Lengkapi Pendaftaran", path: "/siswa/formulir" },
];

const othersItems: NavItem[] = [];

const SiswaSidebar: React.FC = () => {
  ...
  return (
    <aside
      // YOSHH! Di sini juga disuntikkan overflow dan padding bawah
    ...
    >
        ...
        <div className={`py-8 flex ${!isExpanded && !isHovered ? "lg:justify-center" : "justify-start"}`}>
          <Link to="/siswa">
            {isExpanded || isHovered || isMobileOpen ? (
              <h1 className="text-2xl font-bold text-white">PPDB Siswa</h1>
            ) : (
      ...
      <div className="mb-6 border-t border-gray-800 pt-4 mt-auto">
        <ul className="flex flex-col gap-4">
          <li>
            <Link to="/siswa/profile" className={`menu-item group ${isActive("/siswa/profile") ? "menu-item-active" : "menu-item-inactive"}`}>
              <span className={`menu-item-icon-size ${isActive("/siswa/profile") ? "menu-item-icon-active" : "menu-item-icon-inactive"}`}><UserCircleIcon /></span>
              {(isExpanded || isHovered || isMobileOpen) && <span className="menu-item-text text-white">Profil Saya</span>}
            </Link>
          </li>
          <li>
            <button
              onClick={handleLogout}
              className={`w-full menu-item group menu-item-inactive hover:bg-red-900/20 cursor-pointer`}
            >
              <span className={`menu-item-icon-size menu-item-icon-inactive text-red-500`}><PlugInIcon /></span>
              {(isExpanded || isHovered || isMobileOpen) && <span className="menu-item-text text-red-500 text-left">Logout</span>}
            </button>
          </li>
        </ul>
      </div>
